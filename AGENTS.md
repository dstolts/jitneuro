...
```
git -C <KnowledgeRoot> pull --ff-only
git -C <WorkspaceRoot>/HubCentral pull --ff-only
```
...
- Repo root `<RepoRoot>`: the directory containing this file (this jitneuro clone).
- Workspace root `<WorkspaceRoot>`: the parent holding your repo clones (contains `<RepoRoot>` and `<KnowledgeRoot>`).
- KnowledgeRoot `<KnowledgeRoot>`: your local `jit-knowledge` clone (resolution order below).

If `<KnowledgeRoot>` cannot be located, resolve it in this order:
...
```
bash "<KnowledgeRoot>/scripts/knowledge-reboot-handoff.sh" sql-bootstrap --root "<KnowledgeRoot>"
```
...
1. `<WorkspaceRoot>\AGENTS.md` when present
...
- **Resume the canonical lane; do not self-name a checkpoint.** If this session
  ...
  `<WorkspaceRoot>\.sessions\<Lane>.md`). Never write a
