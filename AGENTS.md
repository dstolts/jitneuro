...
```
git -C C:\Users\dstolts\Code\jitneuro pull --ff-only
```
...
```
git -C C:\Users\dstolts\Code\jit-knowledge pull --ff-only
git -C C:\Users\dstolts\Code/HubCentral pull --ff-only
```
...
- Repo root: `C:\Users\dstolts\Code\jitneuro`
- Workspace root: `C:\Users\dstolts\Code`
- KnowledgeRoot: `C:\Users\dstolts\Code\jit-knowledge`

If `C:\Users\dstolts\Code\jit-knowledge` is unavailable, resolve KnowledgeRoot in this order:

1. `KNOWLEDGE_ROOT` (preferred) or `JIT_KNOWLEDGE_ROOT` (legacy)
2. `~/.claude/knowledge-root` pointer
3. `<repo>\.agents\knowledge-root` pointer
4. `<repo>\.cursor\knowledge-root` pointer
5. `~/.claude/workspace.json`
6. `<repo>\.jit-knowledge`
7. `%USERPROFILE%\Code\jit-knowledge`
...
```
bash "C:\Users\dstolts\Code\jit-knowledge/scripts/knowledge-reboot-handoff.sh" sql-bootstrap --root "C:\Users\dstolts\Code\jit-knowledge"
```
...
For any knowledge lookup question, run SQL-backed query first:

```
~/Code/HubCentral/scripts/knowledge-query-curl.sh query --q "<question>" --limit 10
```

If it returns degraded mode or no useful hits, fall back to targeted Git file reads.

## Required Bootstrap

Before substantive work, read:

1. `C:\Users\dstolts\Code\AGENTS.md` when present
