...
**Location:** `.claude/bundles/`, `.claude/engrams/`, the knowledge catalog index (routing)
...
**Fix:** Move the section to a bundle (`.claude/bundles/<domain>.md`). Add a route to the shared catalog index so the bundle loads on-demand when the topic comes up. Remove the section from MEMORY.md entirely.
