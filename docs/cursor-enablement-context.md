...
- **Load:** Resolve session (name or #) → **read** session file → **read** listed bundles → **read** `.the configured shared catalog` (routing) and **MEMORY.md** (project facts; always read file) → update `.current` → report.
