...
- /learn (updates this repo's bundles, engrams; flags new routes for PR to the shared catalog index)
...
- Routing from the shared knowledge catalog index (shared across all users; extensions via PR)
...
- MEMORY.md (auto-memory) is always per-user (`~/.claude/projects/`). It is NOT
  shared between users. Each developer has their own MEMORY.md. Routing is shared via the team's configured knowledge catalog index.
