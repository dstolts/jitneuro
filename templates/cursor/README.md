...
The rule uses `.claude/` for session-state, bundles, and engrams. Resolve `.claude/` from the workspace root or the project root that actually contains it (e.g. in a multi-root workspace, the root that has `session-state/`). Resolve `.jitneuro/` from the active repo when present, and resolve any internal/team shared catalog through that team's configured path or URL resolver.
