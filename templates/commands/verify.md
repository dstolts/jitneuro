...
2. **Check these 9 components** and report status for each:
   ...
   | 9 | Shared catalog resolver | If a shared catalog is configured, check ~/.claude/url-resolver.md has that entry | Entry present or no catalog configured | File missing for configured catalog | Configured catalog entry missing |
