...
e. **On approval:** Write to `.claude/bundles/<name>.md` and suggest
   opening a PR to `the configured shared catalog` to add the routing entry for this bundle.

### 4. Without arguments (`/bundle`):

a. Scan `.claude/bundles/` for actual files. Count lines in each.
b. Read `.the configured shared catalog` (via `~/.claude/url-resolver.md`) and compare -- flag bundles that exist on disk
...
4. On approval: write both files, archive original, suggest PR to the configured shared catalog
...
- Routing entries live in `the configured shared catalog` -- suggest a PR but do not write locally.
