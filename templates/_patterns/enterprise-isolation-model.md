...
- /learn (updates this repo's bundles, engrams; flags new routes for PR to <KnowledgeRoot>/INDEX.md)
...
- Routing from <KnowledgeRoot>/INDEX.md (shared across all users; extensions via PR)
...
- MEMORY.md (auto-memory) is always per-user (the agent's local project directory). It is NOT
  shared between users. Each developer has their own MEMORY.md. Routing is shared via <KnowledgeRoot>/INDEX.md.
