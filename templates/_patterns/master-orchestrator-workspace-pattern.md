---
type: pattern
purpose: Any master agent setting up a multi-repo orchestration session MUST consult this before delegating subagent work -- firing when a session root spans more than one repository -- because skipping it causes the master session to accumulate subagent context directly, exhausting its context window and degrading coordination capacity mid-sprint before the work is complete.
...
MASTER SESSION (workspace root: ~/Code/)
  |-- Brainstem loaded: CLAUDE.md + MEMORY.md (project index) + INDEX.md (routing)
...
**Without this pattern:** You open a terminal in each repo. Each has its own Claude
...
**With this pattern:** One master session holds the full picture. It knows the sprint
...
1. **Route tasks** to the right repo + bundles using the routing index (INDEX.md)
...
The master session has access to all repos underneath. Routing weights in the
workspace MEMORY.md tell it which bundles and engrams to load per task. Commands
(/save, /load, /learn) operate at workspace level, covering all repos.
...
## Repos Involved
- ~/my-api -- branch sprint-comments-001, 4 stories done
- ~/my-frontend -- branch sprint-comments-001, 2 stories done
...
See sub-orchestrator-pattern.md for the full pattern, including:
