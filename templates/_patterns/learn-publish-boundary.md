---
type: pattern
purpose: Defines the boundary a /learn-style memory-capture workflow applies before persisting a captured item -- what stays PERSONAL (business, credential, project, or preference-specific; kept in local memory) versus what is PUBLISHABLE as a universal pattern any user could reuse; read before running or configuring /learn so machine-specific facts are never published and universal patterns are never buried in personal memory.
...
# /learn Capture Boundary: Personal vs Publishable

`/learn` captures corrections, confirmed approaches, and new patterns. Before persisting any of them it must decide whether the item is **personal** (stays in local memory) or **publishable** (a universal pattern worth sharing). This pattern defines that boundary -- the guardrail that keeps machine-specific facts out of shared knowledge and keeps genuinely universal patterns from being buried as personal notes.
