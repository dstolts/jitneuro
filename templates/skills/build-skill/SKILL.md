...
flags, `model` if applicable) AND the jitneuro required fields (`type`,
...
open a PR. Never merge -- jitneuro is PR-gated; Owner approves.
...
- `<destination>` -- project (`.claude/skills/`), personal (`~/.claude/skills/`),
  or jitneuro (`skills/`). Default jitneuro for cross-system skills.
