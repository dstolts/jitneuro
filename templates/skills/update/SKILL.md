...
# Update jit-knowledge (Refresh an Existing Install)

Re-running the installer is the update path. No separate migration script needed.

## When to use

- After `git pull` on the jit-knowledge clone (template files may have changed)
- After bumping the `.jit-knowledge` submodule pin in a consuming repo
- After the jit-knowledge repo tags a new release and your pin advances
- Whenever `~/.claude/hooks/jit-knowledge-session-start.sh` or `~/.claude/rules/jit-knowledge-load.md` drifts from the shipped template
...
```bash
cd "$JIT_KNOWLEDGE_ROOT"   # e.g. <CodeBasePath>\jit-knowledge
git pull --ff-only
```
...
- Refreshes the jit-knowledge clone (step 2 -- already done in step 1 if you pulled manually, but safe to re-run)
- Installs any new per-component node dependencies (step 3)
- Ensures PyYAML is current (step 4)
- Updates `~/.claude/rules/jit-knowledge-load.md` if the shipped template has changed and the local file matches the previous shipped version; warns and preserves local edits if the file diverged (step 5)
- Re-materializes `.claude/CLAUDE.md` inside the clone (step 5b)
- **Overwrites** `~/.claude/hooks/jit-knowledge-session-start.sh` with the repo-canonical version and re-registers it in `~/.claude/settings.json` if not already present (step 5c) -- hook files are always overwritten because the repo copy is canonical
- Updates `workspace.json` with `jit_knowledge_root` if the file exists (step 6)
...
For any downstream repo that consumes jit-knowledge as a `.jit-knowledge/` submodule, bump the pin per `governance/PIN-POLICY.md`:

```bash
cd <consuming-repo>
git -C .jit-knowledge checkout <new-tag-or-sha>
git add .jit-knowledge
git commit -m "chore(jit-knowledge): bump pin to <new-tag>"
```
...
- `~/.claude/rules/jit-knowledge-load.md` if the local copy diverges from the shipped template (preserved with a WARN; merge manually)
...
The hook file (`~/.claude/hooks/jit-knowledge-session-start.sh`) IS always overwritten because it is a mechanical artifact that must match the repo version exactly. If you have made local edits to the hook, re-apply them after the update or upstream them to jit-knowledge via a PR.
