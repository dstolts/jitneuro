...
description: 'Route a captured finding (lesson, rule, pattern) to its correct durable home and deploy it -- a jitneuro branch+PR, a local file write, or a Hub.md append -- then audit-log it and POST it to the WS8 dashboard. Use after the master or /learn has classified a finding''s type, destination, and risk tier and needs it routed.'
purpose: BINDING for every master agent or /learn invocation routing a captured finding to its correct durable home; MUST be invoked after the master classifies a finding's breadth and destination -- skipping means the lesson lands in local volatile memory and dies there, never reaching knowledge catalog canonical or the WS8 audit dashboard.
tags: [knowledge-router, recursive-improvement, skill, routing, jitneuro, audit-log, learn]
scope: public
departments: [all]
read_when: After the master or /learn classifies a finding and needs to deploy it to its correct durable home in jitneuro, local memory, or Hub.md.
...
to its correct home: a jitneuro branch+PR, a local direct write, or a Hub.md
...
- **Master classifies; runner deploys.** The master (or `/learn`) provides ALL
  ...
  a guess.
- **jitneuro is PR-gated.** For `destination: jitneuro`, the runner opens
...
- **Frontmatter VALIDATED, never scaffolded.** For `destination: jitneuro` and
...
- **Audit log is append-only + committed.** `knowledge-audit.log.md` at the
  jitneuro repo root is the durable, version-controlled audit trail. For
  `destination: jitneuro` the audit row is appended and committed IN-BRANCH --
...
- **ENV vars for secrets.** The dashboard-API endpoint URL and auth token are read from
  ...
  the public `<your-dashboard-api-hostname>`. The public hostname is Cloudflare-proxied and its
...
- **Invocability.** Model- and user-invocable (Anthropic skill defaults, no flags
  ...
  because every jitneuro deploy is PR-gated (Owner merges) and local writes are
...
|---|---|---|
| `jitneuro` | branch + commit + PR via `gh` (Owner merges) | Yes -- validated, rejected if invalid |
...
Path resolution:
- `destination: jitneuro` -- `slug` is the repo-relative file path (e.g.,
...
| `destination` | yes | string | One of: `jitneuro`, `local`, `hub` |
| `risk_tier` | yes | int | 1 (T1 auto), 2 (T2 veto-window), 3 (T3 approval) |
| `slug` | yes | string | Repo-relative path for jitneuro; `~/`-path for local; label for hub |
...
1. **Load and validate manifest.** Parse JSON. Check all required fields present and
   non-empty. Verify `destination` is one of `{jitneuro, local, hub}`. Exit 1
   on any failure.
...
3. **Check idempotency.** Scan `knowledge-audit.log.md` for a row with matching hash,
   ...
   unmerged deploy PRs.
4. **Validate frontmatter** (jitneuro and local only). YAML-parse the leading
...
6. **Deploy + audit row.**
   - `jitneuro`: create branch `feat/kr-<type>-<slug-fragment>-<YYYYMMDD>`,
...
7. **Audit row format.** One pipe-delimited row:
   ...
   For `jitneuro` the `destination` field is the target file path; for `local`
...
8. **POST to dashboard-API.** `POST $KNOWLEDGE_API_URL/api/knowledge` with the row data
   ...
   - Git/PR fields (`jitneuro` destination only): `repo` (`"<your-org>/<knowledge-catalog-repo>"`),
     `branch` (deploy branch name), `commit_sha` (deploy commit SHA), `pr_url` (PR URL),
     `pr_number` (integer parsed from PR URL).
   - Content diff: `diff_content` -- for `jitneuro` = `git show <sha> --format=`;
...
- `gh` CLI authenticated to `<you>/jitneuro` (for `destination: jitneuro`)
- `KNOWLEDGE_API_URL` and `KNOWLEDGE_API_TOKEN` env vars (for dashboard-API POST; optional)
- Read/write access to the jitneuro clone
...
result = subprocess.run(
    ...
    cwd="<CodeBasePath>/jitneuro",  # resolve <CodeBasePath> per governance/path-conventions.md
