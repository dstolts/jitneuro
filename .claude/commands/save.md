...
6. Re-run the reboot-safety gate and re-open the saved checkpoint before
   ...
   of claiming success.
7. After the final gate and checkpoint readback, persist the knowledge reboot
   handoff. This is required so `/load <name>` performs the machine-wide
   knowledge reconciliation before it resumes the session:

   ```bash
   bash <KnowledgeRoot>/scripts/knowledge-reboot-handoff.sh prepare --session <name> --root <KnowledgeRoot>
   ```

   Do not claim the reboot handoff is complete if this command fails.
8. The final save output must print these fields plainly:
...
The final `--reboot` message must state: **"Knowledge will be reconciled on
the next /load. Exit this client completely, then restart and run: /load
<name>."** A context clear or a new turn in the same client is not a reboot;
the complete exit makes the startup hooks and the reboot handoff visible.
