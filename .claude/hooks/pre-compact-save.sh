...
#
# Config: .claude/knowledge.json (or legacy jitneuro.json) key hooks.preCompactBehavior: "warn" | "block"
...
CONFIG="$(dirname "$HOOKS_DIR")/knowledge.json"; [ -f "$CONFIG" ] || CONFIG="$(dirname "$HOOKS_DIR")/jitneuro.json"
